            ...
            for tag in element.iter("tag"):
                if tag.attrib['k']:
                    ...
                    if isinstance(eval_k, list):
                        eval_k = eval_k[0]
                    if isinstance(eval_v, list):
                        eval_v = eval_v[0]
                    
                    if "tiger:" in eval_k:
                       eval_k = eval_k.split("tiger:")[1:]
                    for search_error in MAPPING:
                        if search_error in tag.attrib['v']:
                            eval_v = re.sub(r'\b' + search_error + r'\b\.?', MAPPING[search_error], tag.attrib['v'])
                    ...
                    if "zip" in eval_k or eval_k == "name" or eval_k == "county":
                        if "zip" in eval_k:
                            if redun_zip == False:
                                redun_zip = True
                                address_node["zip code"] = eval_v
                                node["address"] = address_node
                        elif eval_k == "name" or eval_k == "county":
                            address_node[eval_k] = eval_v
                            node["address"] = address_node
 '''
