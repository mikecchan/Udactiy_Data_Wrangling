...
def process_file(filename, fields):
    ...
    data = []
    classification = [  "kingdom",   "family",  "order",   "phylum",  "genus",  "class"]
            ...
            for key, value in fields.iteritems():
                ...
                if value == 'synonym':
                    if row[value] == 'NULL':
                        ...
                        row[value] = parse_array(row[value])
                        
                if value == 'name':
                    if row[value] == 'NULL' or not row[value].isalnum():
                        row[value] = row['label']
                ...
                if row[value] != None:
                    ...
                    if isinstance(row[value], list):
                        ...
                        # loop through list
                        # and strip each item
                        # then re-assign row[value] to the new cleaned list
                            item = item.strip()
                            new_list.append(item)
                        row[value] = new_list
                    else:
                        # else, if row[value] is not a list
                        row[value] = row[value].strip()
  
                if value in classification:
                    classDir[value] = row[value]
                    del row[value]
                    
            row['classification'] = classDir
            data.append(row)
        pass

    return data
"""
def check_not_alphanum(value):
    try:
        isalnum(value)
        return False
    except ValueError:
