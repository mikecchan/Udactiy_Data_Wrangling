...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for i in range(3):
            l = reader.next()
        # YOUR CODE HEREfor row in reader:
            # Loops through each row in the reader
        for row in reader:
            newdict = {}
            if row["binomialAuthority_label"]!='NULL':
                if '|' in row["binomialAuthority_label"]:
                    row["binomialAuthority_label"] = row["binomialAuthority_label"].split('|')[1][0:]
                    row["binomialAuthority_label"] = row["binomialAuthority_label"].strip('}')
                         
                if '(' and ')' in row['rdf-schema#label']:
                    row['rdf-schema#label'] = row['rdf-schema#label'].split('(')[0][0:]
...
def update_db(data, db):
    ...
    for a in data:
         db.arachnid.update({"label":a.keys()[0]},{"$set": {"classification.binomialAuthority":a.values()[0]}})
    return db
