...
def add_field(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for row in reader:
            for column in row:
                if column in process_fields:
                    if column == "binomialAuthority_label":
                        if row[column] == 'NULL':
                            continue
                        elif '|' in row[column]:
                            row[column] = row[column].split('|')[1][0:]
                            row[column] = row[column].strip('}')
                        data['binomialAuthority'] = row[column]
                    if column == 'rdf-schema#label':
                        if '(' and ')' in row[column]:
                            row[column] = row[column].split('(')[0][0:]
                        data['label'] = row[column]
